...
function PlayState:enter(params)
    ...
    self.player = Player({
        x = (self.level.landingTileX - 1) * TILE_SIZE, y = 0,
        width = 16, height = 20,
        texture = 'green-alien',
...
function PlayState:update(dt)
    ...
    -- if the player has acquired the key, then display on the hud in play state the key's status
    self.key = self.player.key
