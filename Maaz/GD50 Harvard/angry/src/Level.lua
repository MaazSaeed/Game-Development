...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        ...
        -- if the player has not collided with any of the bodies in the game world e.g., ground, obstacles, or any other thing
        ...
        if love.keyboard.wasPressed('space') and not self.playerCollided then
            ...
            alien1.body:setLinearVelocity(xVel + xShift, yVel + yShift)
            alien2.body:setLinearVelocity(xVel + xShift, yVel - yShift)

            alien2.body:setAngularDamping(1)
            alien1.body:setAngularDamping(1)
            
            table.insert(self.alienBuddies, alien1)
            table.insert(self.alienBuddies, alien2)

            self.playerCollided = true
        end

        local aliens = 0
        for k, alien in pairs(self.alienBuddies) do
            local x, y = alien.body:getPosition()
            local dx, dy = alien.body:getLinearVelocity()

            if x < 0 or (math.abs(dx) + math.abs(dy) < 1.5) then
                alien.body:destroy()
                table.remove(self.alienBuddies, k)
                aliens = aliens + 1
            end
        end

        if aliens == #self.alienBuddies then
