...
function PlayState:calculateMatches()
    ...
    if matches then
        -- +5 seconds for the player on finding a match
        ...
        for k, match in pairs(matches) do
            -- 3 tiles in each match, so we can use any of the three to get the variety e.g., triangle, cross, flat etc.
            local variety = match[1].variety 
            local increment = variety * 10
            self.score = self.score +  #match * increment
        end
        ...
        Timer.tween(1.0, tilesToFall):finish(function()
