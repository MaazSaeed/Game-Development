...
This technique in which we are traversing from face to face, edge to edge, vertex to vertex, only makes senes of the mesh is manifold. This only works with the assumption that there is a single loop of triangles going around the vertex.
...
Each stores local neighbourhood information.

There are similar tradeoffs to similar polygon list:
- cons: additional storage, incoherent memory access
- pros: better access time for individual elements, intuitive traversal of local neighbourhoods
...
Common modeling paradign in modern 3D tools:
- Coarse "control cage"
